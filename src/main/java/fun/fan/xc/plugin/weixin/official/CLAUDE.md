# 微信公众号模块 (fun.fan.xc.plugin.weixin.official)

## 模块职责

该模块提供了微信公众号相关的API接口和管理功能，包括Access Token管理、JS Ticket管理、API调用等。

## 核心组件

### API接口类
- `OfficialWeiXinApi`: 公众号API实现类，继承自BaseWeiXinApi
- `OfficialWeiXinApiClient`: 公众号API客户端接口

### 控制器类
- `OfficialWeiXinApiController`: 公众号API控制器，处理HTTP请求
...
### API实现类
- `OfficialWeiXinApi.kt`: 公众号API实现类，提供了以下功能：
  - 获取Access Token
  - 获取JS Ticket
  - 发送客服消息
  - 发送模板消息
...
### API客户端接口
- `OfficialWeiXinApiClient.kt`: 公众号API客户端接口，定义了与微信服务器交互的方法

### 控制器类
- `OfficialWeiXinApiController.kt`: 公众号API控制器，处理以下HTTP请求：
  - 微信服务器验证
  - 消息接收和处理
  - OAuth2授权回调

### Token管理类
- `OfficialAccessTokenManager.kt`: 公众号Access Token管理器，负责：
  - Access Token的获取和刷新
  - 继承自WeiXinBaseTokenManager
  - 实现doRefresh方法调用requestToken

- `OfficialJsApiTicketManager.kt`: 公众号JS Ticket管理器，负责：
  - JS Ticket的获取和刷新
  - 继承自WeiXinBaseTokenManager
  - 实现doRefresh方法调用requestToken
