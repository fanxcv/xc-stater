...
abstract class AbstractTokenManager : TokenManager {
        ...
        try {
            ...
            if (entity.updateCount.get() == updateCount.get()) {
                ...
                if (entity.expires == 0L || entity.expires <= now) {
                    ...
                    refresh(entity)
                    log.info("===> weixin: {}: sync refresh finish", logName())
                    ...
                    runBlocking {
                        coroutineScope {
                            launch {
                                refresh(entity)
                                log.info("===> weixin: {}: async refresh finish", logName())
                            }
                        }
                    }
                }
                ...
                log.warn(
                    "===> weixin: {}: AtomicInteger 值不相同, 不做更新操作, Global: {}, Entity: {}",
                    logName(), updateCount.get(), entity.updateCount.get()
                )
            }
        } finally {
            lock.unlock()
    ...
    protected fun updateToken(entity: TokenEntity, json: String, key: String) {
        ...
        log.info("{} 获取到新的Token: {}, 到期时间: {}", logName(), entity.token, entity.expires)
    }
     ...
     * 日志标志
     */
    protected abstract fun logName(): String
}
